<template>
  <div class="container main pt-5 pb-5" id="book-list">
    ...
    <SimpleSearch
      ...
      :action="
        url => {
          location.href = `/${this.$l1.code}/${this.$l2.code}/book/list?url=${encodeURIComponent(url)}`
        }
      "
      ref="search"
      class="mb-5"
    ...
    <ul class="list-unstyled booklist">
      <li v-for="book in booklist" class="booklist-item text-center">
        <a :href="`/${$l1.code}/${$l2.code}/book/index?url=${encodeURIComponent(book.url)}`" class="link-unstyled">
          <img
            :src="
              book.thumbnail
